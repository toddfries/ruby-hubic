...
class Hubic
    ...
    def mkdir(obj, parents: false)
        ...
        if (meta = get_metadata(obj)).nil?
            ...
            raise "mkdir '#{path}': EEXIST a file already exists"
        elsif !parents
            puts "mkdir '#{path}': EEXIST the directory already exists"
        end
    end
    ...
    def upload(src, obj, type=TYPE_OCTET_STREAM, &block)
        #puts "upload(#{src},#{obj[:name]},#{type}): start"
        case src
        ...
        then
            ...
            newobj = obj
            newobj[:name] = "#{obj[:name]}/#{srcbn}"
            puts "NOTICE: #{obj[:name]} is a dir, changing destination to #{newobj[:name]}"
            obj = newobj
	    upload(src, obj, type)
            return
        else
            mysize = meta[:size]
            puts "WARNING: over-writing existing file #{obj} of #{mysize} bytes (#{meta})"
        end
        begin
